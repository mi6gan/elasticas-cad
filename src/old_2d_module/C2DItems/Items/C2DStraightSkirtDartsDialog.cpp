...
C2DStraightSkirtDartsDialog::C2DStraightSkirtDartsDialog( const QString & name,
                                              const QString & prefix,
                                              QPair< QString, double > units,
                                              QGraphicsScene * scene,
                                              QObject * parent )
    : C2DItemDialog( name, prefix, units, scene, parent )
{
    this->dialog_grid = new QGridLayout( this );
    //this->dialog_grid->addWidget( this->dialog_view, 0, 0, 9, 1 );
    ...
    for ( int i = 0; i <= 2; i++ )
    {
        ...
        this->dialog_grid->addWidget( label[ i ], 2 * i + b, 3 );
        this->dialog_grid->addWidget( spins[ i ], 2 * i + b, 4 );

        if ( i == 0 )
            for ( int j = 0; j <= 2; j++ )
                this->dialog_grid->addWidget(
        ...
        for ( int j = 0; j <= 3; j++ )
        {
            ...
            this->dialog_grid->addWidget( group, 2 * i + 2, j + 1 );
        ...
        this->dialog_grid->addWidget( this->top_dialog_button_box, 7, 1, 1, 4 );
        this->dialog_grid->addWidget( this->bottom_dialog_button_box, 8, 1, 1, 4 );
    }
    this->valueChanged();
...
C2DStraightSkirtDartsDialog::~C2DStraightSkirtDartsDialog()
{

}

void C2DStraightSkirtDartsDialog::valueChanged()
{
    for ( int i = 0; i <= 2; i++ )
        for ( int j = 0; j <= 2; j++ )
...
void C2DStraightSkirtDartsDialog::Apply()
{
    ...
    for ( int i = 0; i <= 2; i++ )
    {
        ...
        {
        case ( 0 ):
            first_name = QString( "%1SideDarts" ).arg( this->prefix() );
            break;
        case ( 1 ):
            first_name = QString( "%1FrontDarts" ).arg( this->prefix() );
            break;
        case ( 2 ):
            first_name = QString( "%1BackDarts" ).arg( this->prefix() );
            break;
        }
...
void C2DStraightSkirtDartsDialog::Reject()
{
...
void C2DStraightSkirtDartsDialog::Reset()
{
...
QList< QList< double > > C2DStraightSkirtDartsDialog::defaults()
{
//    const Elasticas::CommonData & var = Elasticas::Core::Instance()->CommonVariables();
    ...
    //Measures
    double T7  = 0.0f; //var["T7"].toDouble() * this->modifier();
    double T9  = 0.0f; //var["T9"].toDouble() * this->modifier();
    double T18 = 0.0f; //var["T18"].toDouble() * this->modifier();
    double T19 = 0.0f; //var["T19"].toDouble() * this->modifier();
    double P18 = 0.0f; //var[ this->prefix() + "P18" ].toDouble() * this->modifier();
    double P19 = 0.0f; //var[ this->prefix() + "P19" ].toDouble() * this->modifier();
    double T46 = 0.0f; //var["T46"].toDouble() * this->modifier();
    double T86 = 0.0f; //var["T86"].toDouble() * this->modifier();
    double Vyt = 0.0f; //var["Vyt"].toDouble() * this->modifier();
