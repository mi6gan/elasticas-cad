#include "C2DMensjacket.h"
C2DMensjacket::C2DMensjacket() : C2DItem()
{
    this->setPrefix( QString( "Mensjacket" ) );
    this->requiredVariables();
    this->addDialog( new C2DMensjacketDialog( QObject::trUtf8( "Мужской пиджак" ), this->prefix(), QObject::trUtf8( "Параметры БК" ),
                     QPair< QString, double> ( QObject::trUtf8( "см" ), 10.0f ), NULL ) );
}

void C2DMensjacket::Update()
{
    this->setVariables();
    this->UpdatePoints();
    this->DrawOnWidget();
    this->MSleeveUpdatePoints("SL:",P["331"]+QPointF(P["971"].x() - P["331"].x() + 200.0f,0));
    this->MSleeveDrawOnWidget("SL:");
    //   C2DSleeve * sleeve = new C2DSleeve;
 //   sleeve->Update();
}
void C2DMensjacket::requiredVariables()
{
...
void C2DMensjacket::setVariables()
{
    H( "Tx" ) = 1.0f + D( "PTx" );
    H( "Ty" ) = 1.0f + D( "PTy" );
...
///// ПЕРЕМЕННЫЕ
   ...
   // H( "P343" ) = 0.0f;
    ...
    H( "P471-1" ) = 4.0f;//ГРАДУСЫ
    H( "P13") = 0;  //П33-13
    H( "P14") = 0; // П35-15
    H( "P7") = 0; // П33-35
}

void C2DMensjacket::UpdatePoints()
{
    QLineF line1,line2;
    QPointF p;
...
//ВТОРАЯ СТРАНИЦА-№81 *******************************************
    ...
    P["123-1"] = p;
//ТРЕТЬЯ СТРАНИЦА-№81 *******************************************
    P["471"] = P["47"] + QPointF( 0.24f * LD("LT18")/2
               - 0.5f *(LD("LT45")/2+LD("LT15")/2-8-LD("LT14")/2),0.0f );//МУЖ
    P["46"] = P["471"] - QPointF( LD("LT46"), 0.0f );
    P["471-1"] = Unicus::rPoint(P["471"],P["46"],LD("P471-1"));
    line1.setPoints(P["46"],P["471-1"]);
    line1.setAngle(line1.angle()-90.0f);
    line1.setLength(LD("LT36")-LD("LT35"));
    P["36"] = line1.p2();
    line2.setPoints(P["471-1"],P["471-1"]+QPointF(1,0));
    line2.setAngle(line1.angle());
    line2.setLength(line1.length());
    P["371"] = line2.p2();
    line1.setPoints(P["36"],P["371"]);
    line1.setLength(LD("LT35")-LD("LT34"));
    P["372"] = line1.p2();
...
////Рассчет точек для сплайнов
    ...
    line1.setLength( QLineF(P["97"],P["941"]).length()/3);
    P["97l"] = line1.p2();
...
//P346
    ...
    P["571"] = QPointF(P["471"].x(),P["57"].y());
    P["972"] = QPointF(P["471"].x(),P["941"].y());
    P["971"] = QPointF(P["471"].x(),P["97"].y());

    Unicus::console( ">>>>>47-471 ->", QLineF(P["471"],P["47"]).length());

}

void C2DMensjacket::DrawOnWidget()
{
    ///////////////////////MENSJACKET
    this->point( Sync( attr( SYNC_NAME, QString( "471" ) ),
                      attr( SYNC_TYPE, C2DSync::Point ),
                      attr( SYNC_LIST, MakeList( "471" ) ),
                      LM->thick_solid() ), true );
    ///////////////////////////////////////////////////
...
//------SPLINES
   ...
   //                     LM->bold_solid() ), true );
    ...
    //MEN'S JACKET PART
    this->line( Sync( attr( SYNC_NAME, QString( "971-471" ) ),
                      attr( SYNC_TYPE, C2DSync::Line ),
                      attr( SYNC_LIST, MakeList( "971","471" ) ),
                      LM->bold_solid() ), true );
    this->line( Sync( attr( SYNC_NAME, QString( "971-97" ) ),
                      attr( SYNC_TYPE, C2DSync::Line ),
                      attr( SYNC_LIST, MakeList( "971","97" ) ),
                      LM->bold_solid() ), true );
    this->line( Sync( attr( SYNC_NAME, QString( "57-571" ) ),
                      attr( SYNC_TYPE, C2DSync::Line ),
                      attr( SYNC_LIST, MakeList( "57","571" ) ),
                      LM->thick_solid() ), true );
    this->line( Sync( attr( SYNC_NAME, QString( "46-471" ) ),
                      attr( SYNC_TYPE, C2DSync::Line ),
                      attr( SYNC_LIST, MakeList( "46","471" ) ),
                      LM->bold_solid() ), true );
    this->line( Sync( attr( SYNC_NAME, QString( "471-1-46" ) ),
                      attr( SYNC_TYPE, C2DSync::Line ),
                      attr( SYNC_LIST, MakeList( "471-1","46" ) ),
                      LM->bold_solid() ), true );
    this->line( Sync( attr( SYNC_NAME, QString( "471-1-371" ) ),
                      attr( SYNC_TYPE, C2DSync::Line ),
                      attr( SYNC_LIST, MakeList( "471-1","371" ) ),
                      LM->bold_solid() ), true );

    this->line( Sync( attr( SYNC_NAME, QString( "371-36" ) ),
                      attr( SYNC_TYPE, C2DSync::Line ),
                      attr( SYNC_LIST, MakeList( "371","36" ) ),
                      LM->bold_solid() ), true );
    this->line( Sync( attr( SYNC_NAME, QString( "371-1-36" ) ),
                      attr( SYNC_TYPE, C2DSync::Line ),
                      attr( SYNC_LIST, MakeList( "371-1","36" ) ),
                      LM->bold_solid() ), true );
    this->line( Sync( attr( SYNC_NAME, QString( "21-411" ) ),
                      attr( SYNC_TYPE, C2DSync::Line ),
                      attr( SYNC_LIST, MakeList( "21","411" ) ),
                      LM->bold_solid() ), true );
    this->line( Sync( attr( SYNC_NAME, QString( "511-411" ) ),
                      attr( SYNC_TYPE, C2DSync::Line ),
                      attr( SYNC_LIST, MakeList( "511","411" ) ),
                      LM->bold_solid() ), true );
    this->line( Sync( attr( SYNC_NAME, QString( "511-911" ) ),
                      attr( SYNC_TYPE, C2DSync::Line ),
                      attr( SYNC_LIST, MakeList( "511","911" ) ),
                      LM->bold_solid() ), true );
}
void C2DMensjacket::DrawSpline( QStringList splinename, QVariant list[] )
{
...
void C2DMensjacket::MSleeveUpdatePoints( QString prefx, QPointF p0 )
{
    QHash< QString, QPointF > SP;
    QLineF line1(P["33"],P["35"]), line2;
    /////Рассчетные параметры проймы
    double N = 0.07f,
           DP = 0.95f * LD("LT38") + ( LD("P13") + LD("P14") ) + 0.57f * LD("LT57") +
            2 * QLineF(P["33"],P["331"]).length(),
           POR = N * DP,
            DOR = ( 1 + N ) * DP;

    SP["P0"] = p0;
    SP["331"] = SP["P0"];
    SP["351"] = QPointF( SP["331"].x() + line1.length() ,SP["331"].y() );

    SP["341"] = QPointF( SP["331"].x() + 0.62f * line1.length() ,SP["331"].y() );
    SP["346"] = QPointF( SP["351"].x() - 0.38f * line1.length() ,SP["351"].y() );
    SP["332"] = QPointF( SP["331"].x() ,SP["331"].y() + 0.62f * line1.length() );
    SP["342"] = Unicus::CircCirc( SP["332"], 0.62f * line1.length(),
                                  SP["341"], 0.62f * line1.length(), true  );

    SP["352"] = QPointF( SP["351"].x() ,SP["351"].y() + 0.38f * line1.length() );
    SP["343"] = Unicus::CircCirc( SP["352"], 0.38f * line1.length(),
                                  SP["346"], 0.38f * line1.length(), false  );

    SP["333"] = P["351"] - QPointF( LD("LT57")+40.0f,0);
    double SHOR = QLineF(SP["333"],SP["351"]).length();

    SP["13"] = QPointF(SP["333"].x(),SP["333"].y() + 0.885f*DOR*sqrt(0.25f -  pow( SHOR/DOR, 2 ))   );
    SP["14"] = QPointF( SP["13"].x() + 0.45f * QLineF(SP["351"],SP["333"]).length(), SP["13"].y() );
    SP["141"] = QPointF( SP["13"].x() + 0.73f * QLineF(SP["351"],SP["333"]).length(), SP["13"].y() );
    SP["15"] = QPointF( SP["351"].x(), SP["14"].y());
    SP["141-1"] = Unicus::rPoint(SP["141"],SP["15"],180.0f);
    SP["353"] = QLineF(SP["141-1"],SP["343"]).pointAt(0.5f);
    SP["142"] = QLineF(SP["141"],SP["352"]).pointAt(QLineF(SP["141"],SP["15"]).length()/
                                                    QLineF(SP["141"],SP["352"]).length());
    SP["143"] = QLineF(SP["14"],SP["141"]).pointAt(0.5f);
    SP["131"] = QLineF(SP["13"],SP["333"]).pointAt(0.25f);
    SP["132"] = QLineF(SP["13"],SP["333"]).pointAt(0.25f);
...
void C2DMensjacket::MSleeveDrawOnWidget( QString prefx )
{
    this->line( Sync( attr( SYNC_NAME, QString( prefx + "331-351" ) ),
                      attr( SYNC_TYPE, C2DSync::Line ),
                      attr( SYNC_LIST, MakeList( prefx + "331", prefx + "351" ) ),
                      LM->thick_solid() ), true );
}
